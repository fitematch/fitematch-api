...
describe('UpdateUserMapper', () => {
  describe('toResponse', () => {
    it('should apply masks to birthday, documents and contacts', () => {
      const result = UpdateUserMapper.toResponse({
        id: 'user-id-1',
        ...
        candidateProfile: {
          documents: {
            rg: { number: '12345678x', issuer: 'SSP', state: 'SP' },
            cpf: { number: '12345678901' },
            cref: { number: '123456gsp', category: 'SP', isActive: true },
            passport: {
              number: 'ab-12.3456',
              country: 'Brasil',
      ...
      expect(result.recruiterProfile?.contacts?.phone?.number).toBe(
        '(21) 98765-4321',
      );
    });
  });
